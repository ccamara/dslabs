...
authors_summary = (
    authors_df
    .groupby(['author_text'], as_index=False)
...
top_authors = authors_summary.nlargest(20, 'n').reset_index(drop=True)
...
new_authors = list(set(authors_summary['author_text'].to_list()).difference(qid_df['author_text'].to_list()))
...
if len(new_props) > 0:
    ...
    # Append new authors and QIDs to existing dataframe and save it.
    wikidata_props_df = pd.concat([qid_df, wikidata_props_df_new]).reset_index(drop=True)
    
    wikidata_props_df.to_csv(authors_qids_csv, index=False)

wikidata_props_df
```

```{python}
#| label: authors_properties
#| eval: false
WIKIDATA_SEARCH_URL = "https://www.wikidata.org/w/api.php"
WIKIDATA_SPARQL_URL = "https://query.wikidata.org/sparql"
HEADERS = {"User-Agent": "dslabs-wikidata-query/1.0 (contact: youremail@example.com)"}
...
def get_wikidata_props(qid: str) -> Dict[str, Optional[str]]:
    ...
    if resp.status_code != 200:
        return {"gender": None, "country_of_birth": None, "country_of_citizenship": None, "ethnicity": None, "sexual_orientation": None}
    results = resp.json().get("results", {}).get("bindings", [])
    if not results:
        return {"gender": None, "country_of_birth": None, "country_of_citizenship": None, "ethnicity": None, "sexual_orientation": None}
    row = results[0]
    ...
    return {
        ...
        "country_of_citizenship":  get_label("countryCitizenshipLabel"),
...
authors_metadata = get_author_wikidata_info(authors_summary['author_text'])

# Merge mapping back into original df (left join)
authors_summary_metadata = pd.merge(authors_summary, authors_metadata, left_on="author_text", right_on='author_name', how="left").copy()


authors_summary_metadata
```

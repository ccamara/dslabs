...
alt.Chart(books_metadata_df).mark_area().encode(
    x="finish_date",
...
alt.Chart(books_metadata_df).mark_bar().encode(
...
```{python}
books_gender_year = (
    books_metadata_df[['finish_date', 'gender', 'pages']]
    .copy()
    .assign(year=books_metadata_df['finish_date'].dt.year)  # extract year
    .groupby(['year', 'gender'])
    .agg(
        books=('pages', 'size'),               # number of rows = books
        pages_sum=('pages', 'sum')             # total pages per year
...
alt.Chart(books_gender_year).mark_area().encode(
    x="year",
    y=alt.Y("gender").stack("normalize"),
    color="gender"
)


alt.Chart(books_gender_year).mark_bar().encode(
    color=alt.Color('gender'),
    x='year',
    y=alt.Y('count()', stack='normalize')
)
...
# Aggregate unique authors per country_of_birth
authors_country = (
    books_metadata_df[['author_text', 'country_of_birth']]
    .dropna(subset=['country_of_birth'])
    .drop_duplicates()
    .groupby('country_of_birth', as_index=False)
    .agg(authors=('author_text', 'size'))
)

# World topojson features
countries = alt.topo_feature(data.world_110m.url, 'countries')

# Choropleth: lookup by feature property 'properties.name' -> your country names
choropleth = (
    alt.Chart(countries)
    .mark_geoshape(stroke='white')
    .transform_lookup(
        lookup='properties.name',
        from_=alt.LookupData(authors_country, key='country_of_birth', fields=['authors'])
    )
    .encode(
        color=alt.Color('authors:Q', title='Number of authors', scale=alt.Scale(scheme='greens')),
        tooltip=[alt.Tooltip('properties.name:N', title='Country'), alt.Tooltip('authors:Q', title='Authors')]
    )
    .project('equirectangular')
    .properties(width=900, height=500, title='Number of Authors by Country (country_of_birth)')
)
...
Or books written in different languages

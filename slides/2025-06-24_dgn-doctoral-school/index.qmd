...
-   New types appear from time to time

![Replace: Forence's nightingale's coxcomb](assets/Screenshot%202023-04-05%20at%2010-03-45%20Five%20charts%20that%20changed%20the%20world%20-%20BBC%20Ideas.png)
:::
:::::
...
```{r}
plot(datasaurus$x, datasaurus$y)
```
...
datasaurus$category <- as.factor(as.integer(as.factor(datasaurus$dataset)))

datasaurus <- as_tibble(datasaurus)
...
If you have good sight, you may be able to see something, but still difficult. Let's try something different: let's create small multiples per category (i.e., an array of independent plots for each category)
